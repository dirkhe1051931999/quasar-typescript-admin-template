<template>
  <div v-if="!route.meta || !route.meta.hidden" class="sidebar-item">
    <q-list
      class="q-list"
      v-if="!route.children || (route.children && route.children.length === 1)"
    >
      <q-expansion-item
        ...
        :header-inset-level="
          route.children && route.children.length === 1 && !route.meta.nested
            ? 0
            : route.meta.fatherPath
            ? 1
            : 0.5
        "
        :class="
          $route.path === resolvePath(route)
            ? 'bg-small-blue text-primary active-tab'
            : 'text-black2'
        "
      ></q-expansion-item>
    </q-list>
...
export default class SidebaritemComponents extends Vue {
  @Prop({ required: true, default: {} }) private route!: RouteRecordRaw;
  ...
  onRouteChange(newVal: any) {
    this.$nextTick(() => {
      ...
      if (
        newVal.meta.fatherPath &&
        newVal.matched[1].name &&
        this.$refs[newVal.matched[1].name]
      ) {
  ...
  private iconName(route: any) {
    return !route.children ||
      (route.children.length === 1 && route.meta && route.meta.icon)
      ? route.meta.icon
